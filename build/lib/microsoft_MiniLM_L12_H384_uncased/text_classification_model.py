...
class TextClassificationModel:
    def __init__(self, model_name='microsoft/MiniLM-L12-H384-uncased', cache_dir=None):
        """
        Initializes the model, tokenizer, and device.
        :param model_name: Name of the pre-trained MiniLM model (default: microsoft/MiniLM-L12-H384-uncased)
    ...
    def tokenize_data(self, texts):
        """Tokenizes the text data using the MiniLM tokenizer."""
    ...
    def train(self, text_train, y_train, text_test, y_test, save_model_path='./saved_model'):
        ...
        model = AutoModelForSequenceClassification.from_pretrained(self.model_name, num_labels=len(set(y_train)), cache_dir=self.cache_dir).to(self.device)
    ...
    def print_class_wise_metrics(self, eval_results):
        ...
        """
        precision = eval_results.get('precision_per_class', [])
        recall = eval_results.get('recall_per_class', [])
        f1 = eval_results.get('f1_per_class', [])
        support = eval_results.get('support_per_class', [])

        if not precision or not recall or not f1 or not support:
            print("Metrics per class are missing.")
            return
    ...
    def predict(self, text_test, y_test, save_predictions_path, save_model_path):
        ...
        model = AutoModelForSequenceClassification.from_pretrained(save_model_path, num_labels=len(self.class_mapping)).to(self.device)
    ...
    # The rest of the methods remain unchanged except for replacing BertForSequenceClassification with AutoModelForSequenceClassification
